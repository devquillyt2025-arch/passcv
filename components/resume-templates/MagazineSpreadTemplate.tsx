...
export default function MagazineSpreadTemplate({ data, sectionOrder, builderDesign }: TemplateProps) {
  ...
  const divider = `1px solid ${pal.rule}`;
      ...
      const renderSection = (id: string) => {
        ...
        switch(id) {
          case 'summary': return data.summary ? (<div key={id}><p style={{ fontSize: 13, color: pal.inkMuted, margin: '16px 0 22px', columnCount: 2, columnGap: 28 }}>{data.summary}</p></div>) : null;
