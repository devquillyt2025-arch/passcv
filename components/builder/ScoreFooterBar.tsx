...
function MetricItem({ metric }: { metric: Metric }) {
  ...
  return (
    ...
    >
      <div
        className="flex flex-col gap-1.5 rounded-lg border border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800 px-3 py-2 transition-all hover:border-slate-300 dark:hover:border-gray-600 hover:bg-white dark:hover:bg-gray-700 hover:shadow-sm cursor-default"
      >
        {/* Header with label and fraction */}
        <div className="flex items-center justify-between gap-2">
          <span className="text-[10px] font-bold uppercase tracking-[0.08em] text-slate-500 dark:text-gray-400">{metric.label}</span>
          <span className="text-[16px] font-bold text-slate-900 dark:text-gray-100 tabular-nums leading-none">
            {metric.value}/{metric.max}
          </span>
        </div>

        {/* Progress bar */}
        <div className="h-1 w-full rounded-[2px] bg-slate-200 dark:bg-gray-700 overflow-hidden">
          <div
      ...
      <PortalPopover isOpen={isHovered && metric.details.length > 0} anchorRef={ref} position="top-center" offset={8}>
        <div className="z-40 w-80 rounded-lg border border-slate-200 dark:border-gray-700 bg-white dark:bg-[#1e293b] p-3 shadow-lg pointer-events-none">
          <div className="space-y-2">
            <p className="text-xs font-semibold text-slate-950 dark:text-gray-100">{metric.label} Details</p>
            <ul className="space-y-1">
              {metric.details.map((detail, idx) => (
                <li key={idx} className="text-[11px] text-slate-600 dark:text-gray-400 leading-relaxed">
                  • {detail}
                </li>
              ))}
            </ul>
          ...
          {/* Tooltip arrow */}
          <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-white dark:bg-[#1e293b] border-r border-b border-slate-200 dark:border-gray-700 transform rotate-45" />
        </div>
      </PortalPopover>
    </div>
  );
...
export default function ScoreFooterBar({ score }: ScoreFooterBarProps) {
  ...
  return (
    <div className="sticky bottom-0 z-20 border-t border-slate-200 dark:border-gray-800 bg-white dark:bg-[#0a0a0f] shadow-[0_-4px_12px_rgba(0,0,0,0.05)] dark:shadow-[0_-4px_12px_rgba(0,0,0,0.3)]">
